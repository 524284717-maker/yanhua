...
把 risk/ 下线上运行的 patent-s1..s5.html 复制到 tools/legacy/，并只做两类最小改动：

  1. 资源路径：geo-data.js / patent-model.js 改为 ../../../shared/ 相对路径（指向工程共享资源目录，不改变加载内容）；
...
def main():
    ...
    for n in PAGES:
        ...
        s = rep(s, 'src="../shared/geo-data.js"', 'src="../../../shared/geo-data.js"', 1, 's%d 地图数据路径' % n)
        s = rep(s, 'src="../shared/patent-model.js"', 'src="../../../shared/patent-model.js"', 1, 's%d 数据模型路径' % n)
