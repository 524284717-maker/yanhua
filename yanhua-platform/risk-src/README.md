...
> 本目录是 **烟花爆竹全链条数字化预警监管平台**（6 个页面）的工程化源码版。
>
> 源码按「**数据模型（core）→ 兼容导出（compat）→ 渲染框架（render）→ 页面实现（pages）**」四层组织，
> 每个算法单元独立成文件，并在文件头以 `@技术特征` 标注其承载的技术要点。
>
> 源码包与 `../../risk/` 下运行的 6 个页面功能完全等价，
> 等价性由 `tools/equivalence-test.html`（数值等价）与 `tools/dom-diff.sh`（界面等价）自动校验。

---

## 一、系统在做什么

针对烟花爆竹仓储环节，利用**已有的扫码出入库数据 + 车辆定位轨迹数据**，
在**同一条监测时间轴上**依次完成 5 项计算与判定，最终输出「隐蔽囤货」与「账面造假」两类风险结论：
...
关键设计：**所有指标都是「监测时刻 t 的纯函数」**，且风险判定采用
「预计算时间线 + 峰值锁存」而非逐帧增量判断 —— 因此判定结果与帧序、播放速度、页面刷新完全无关
（实现见 `js/core/11-timeline.js`）。
...
├── README.md                      本文件
├── css/
│   └── platform.css               共享样式（深蓝科技感大屏）
├── js/
│   ├── core/                      ★ 数据模型层（按算法单元拆分）
│   │   ├── 00-namespace.js        FWCore 命名空间与模块注册约定
│   │   ├── 01-params.js           S0 阈值参数集 P / 功能时间线 TL / 考察窗口 BOOK_WIN
│   │   ├── 02-geometry.js         平面几何基础函数（路径插值、点距）
│   │   ├── 03-geo-anchor.js       目标仓储地理锚定（GeoJSON 解析、质心 / 外接框）
│   │   ├── 04-op-dataset.js       扫码出入库流水数据集 OP（含无轨迹标记）
...
│   ├── render/
│   │   ├── page-shell.js          页面外壳：等比缩放、监测时钟、主循环、重置
│   │   └── boot.js                页面启动装配
│   └── pages/
│       └── s1.js … s5.js          5 个功能页各自的渲染实现
├── pages/
│   ├── index.html                 系统功能总览
│   └── s1.html … s5.html          5 个功能页（可直接双击打开）
└── tools/
    ├── build-pages.py             由 pages-src/ + css/ 组装 pages/ 与 js/pages/
    ├── pages-src/                 页面正文片段与页面脚本源（★ 页面真源，可编辑）
    ├── build-legacy.py            由 ../../risk/ 生成旧版对照基准（注入时刻冻结钩子）
    ├── equivalence-test.html      ★ 数值等价性自检（新版模块 vs 旧版单文件，t=0~60）
    ├── dom-diff.sh                ★ 界面等价性自检（冻结时刻逐页比对 DOM + ECharts）
...
> 这样可以保证 6 个页面无论从哪一页进入，`DERIVED` 的判定时刻
> （`riskHideFrom` / `bookFrom` / `alertFrom`）完全相同。

顺序的唯一真源是 `tools/build-pages.py` 中的 `CORE_MODULES` 常量。
...
```bash
# 页面互不依赖构建，双击即可打开
open risk-src/pages/index.html
```

页面通过 `<script src>` 以经典脚本方式加载本地模块，**无需打包、无需本地服务器**。
外部资源仅 `../../shared/geo-data.js`（江西省行政区划 GeoJSON，公开数据）。

### 2. 重新组装页面（仅在修改 `tools/pages-src/` 后需要）

```bash
python3 risk-src/tools/build-pages.py
```

直接改 `pages/*.html` 会在下次组装时被覆盖 —— 页面真源是 `tools/pages-src/`。

### 3. 等价性自检（两项，均应 PASS）

```bash
# ① 数值等价：逐时刻比对全部导出量（55,610 项）
open risk-src/tools/equivalence-test.html

# ② 界面等价：5 个页面 × 23 个冻结时刻（115 组），需 agent-browser 在 PATH 中
bash risk-src/tools/dom-diff.sh
```

数值自检页以 `PASS / FAIL` 大字提示结果并列出差异明细；
...
| | `../../risk/`（运行版） | 本源码包 |
| --- | --- | --- |
| 数据模型 | `shared/patent-model.js` 单文件 507 行 | `js/core/` 14 个模块 |
...
运行版页面可继续使用，源码包不修改、不替换 `../../risk/` 下的任何文件。

---

## 六、代码阅读建议

1. `js/core/01-params.js` —— 先看全部阈值参数，这是判定边界的数值来源。
...
配套说明见 `../docs/技术特征对照表.md`（同时提供可读的 `技术特征对照表.html`）。
...
| 接口覆盖 | 运行版每个导出项在源码版同名同类型 | 48 项 | PASS |
| 判定时刻 | 隐蔽囤货 32.0′ / 账面造假 40.6′ / 管控预警 41.0′ | 3 项 | 两版完全一致 |
